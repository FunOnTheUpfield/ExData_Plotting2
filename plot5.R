# plot1.R
# Script 5 of 5 in repo https://github.com/FunOnTheUpfield/ExData_Plotting2
...
f <- ggplot(cityvehicles, aes(year , Emissions)) + geom_point() + facet_wrap(~fips) + ylim(0,500) + geom_smooth(aes(group=1), method="lm", se=FALSE) + ggtitle("Air pollution from motor vehicle use")
print(f)
dev.off()
